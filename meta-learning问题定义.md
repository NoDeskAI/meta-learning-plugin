...
- [`SignalCapture`](src/meta_learning/layer1/signal_capture.py) 按优先级捕获 4 种触发条件（first-match-wins）：`user_correction` > `error_recovery` > `new_tool` > `efficiency_anomaly`。`user_corrections` 优先级最高，但在没有用户纠正的常见情况下，只要存在 `errors_encountered` 就会触发 `error_recovery`，从而吞没同一次任务中的 `new_tool` 和 `efficiency_anomaly` 信号。（注：Layer 2 对 `USER_CORRECTION` 类信号有 fast-track 路径——跳过聚类直接生成 taxonomy entry。）
...
**问题**：Layer 2 流水线有门控条件（默认 ≥2 条 pending signals 或 ≥8h 且有 pending），在实际使用中用户每天可能只触发 1-2 个信号，首次触发仍需等待一定积累。在此期间系统的学习能力受限。

**代码映射**：

- [`Layer2Orchestrator.should_trigger()`](src/meta_learning/layer2/orchestrator.py) 实现了触发逻辑：`min_pending_signals`（默认 2）或距上次运行超过 `max_hours_since_last`（默认 8h）。
